...
void processclient(int sockfd) {
    ...
    char command[1024] = {0};
    char gf[1024]={0};
    printf("Entered procclient\n");
    while (1) {
        memset(buff, 0, sizeof(buff));
        memset(command, 0, sizeof(command));
        memcpy(gf, buff, sizeof(buff));

        int valueread = read(sockfd, buff, sizeof(buff));
        buff[valueread] = '\0';
        ...
        } else if (strcmp(token, "filesrch") == 0) {  //execution of filesrch command
        ...
        } else if (strcmp(token, "tarfgetz") == 0) {  //execution of tarfgetz command
