...
void search_directory(char* path, char* filename, char* command) {
    DIR* dir;                                               // the use of this function is for searching file in all subdirectories 
    struct dirent* entry;
    struct stat st;
...
void processclient(int sockfd) {
    ...
    while (1) {
        ...
        if (token == NULL) {
        ...
        } else if (strcmp(token, "tarfgetz") == 0) { //executiion of tarfgetz command
            ...
            if (size1_str == NULL || size2_str == NULL) {
                ...
                if (size1 < 0 || size2 < 0 || size1 > size2) {
                    sprintf(command, "The size range is invalid. Please try again.\n");
                } else {
                    char root_path[1024];
                    sprintf(root_path, "/home/zala3/");
                    ...
                    if (unzip_flag != NULL && strcmp(unzip_flag, "-u") == 0) {
                        ...
                        system("tar -xzf temp.tar.gz -C /home/zala3/saved");
        ...
        } else if (strcmp(token, "getdirf") == 0) {  // execution of getdirf command
            ...
            if (date1_str == NULL || date2_str == NULL) {
                ...
                // Finding files that match the date range
                char command_buf[BUFSIZE];
        ...
        } else if (strcmp(token, "fgets") == 0) { //execution of fgets command
    ...
    } else if (strcmp(token, "targzf") == 0) { //execution of targzf command
        ...
        char* unzip_flag = strtok(NULL, " ");
         int uzip=0;
        if(strcmp(extension1, "-u")==0 || strcmp(extension2, "-u")==0 || strcmp(extension3, "-u")==0 || strcmp(extension4, "-u")==0 ) 
        uzip=1;
        char root_path[1024];
       sprintf(root_path, "/home/zala3/");
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        if (active_clients < 6 || (active_clients > 13 && active_clients % 2 == 0)) {
            pid_t pid = fork();
            if (pid == -1) {
                perror("fork");
                exit(EXIT_FAILURE);
